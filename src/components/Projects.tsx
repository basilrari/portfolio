...
function ProjectCard({ project, index, featured }: { project: typeof projects[0] | typeof moreProjects[0]; index: number; featured?: boolean }) {
  return (
    <motion.div
      ...
      className="group relative rounded-xl border p-6 transition-all duration-300 focus-within:border-emerald-500/50"
      style={{
        backgroundColor: featured ? 'var(--bg-card)' : 'var(--bg-card-hover)',
      ...
      onMouseLeave={(e) => {
        ...
        (e.currentTarget as HTMLElement).style.backgroundColor = featured ? 'var(--bg-card)' : 'var(--bg-card-hover)';
      }}
    >
      ...
      <div className="flex-1">
        <div className="flex items-start justify-between gap-4 mb-3">
          ...
          <a
            ...
            className="flex-shrink-0 transition-colors duration-200 p-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 rounded"
        ...
        <div className="flex flex-wrap gap-2">
          {project.tech.map((tech) => (
            <span
              key={tech}
              className="px-2.5 py-0.5 rounded-md font-mono text-xs"
              style={{
