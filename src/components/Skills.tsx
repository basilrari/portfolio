  ...
  return (
    <section className="py-20 px-6 bg-slate-900/30">
      <div className="max-w-6xl mx-auto">
        ...
        >
          <p className="font-mono text-emerald-400 text-sm tracking-wider mb-3">02 / Arsenal</p>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {skills.map((skill, index) => (
              <motion.div
                ...
                className={`rounded-xl border p-6 transition-all duration-300 hover:scale-[1.02] ${skill.bgColor} ${skill.borderColor}`}
              >
                <div className={`${skill.color} mb-4`}>
                  {skill.icon}
                </div>
                <h3 className={`text-sm font-semibold mb-4 ${skill.color} uppercase tracking-wider`}>
                  {skill.category}
                </h3>
                <div className="flex flex-wrap gap-2">
                  {skill.items.map((item) => (
                    <span
                      key={item}
                      className="px-2.5 py-1 bg-slate-800/80 text-slate-300 text-xs rounded-md font-mono"
                    >
                      {item}
                    </span>
                  ))}
